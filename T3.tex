...
We define $runs$ and $motifs$ in $T_3$ as follows:

\begin{definition}
\label{run}
A \emph{run} of length $n$ is $n$ consecutive terms which are all identical.
\end{definition}
...
Similar to the computer science notion of \emph{run-length encoding}, we define a function $\Lambda$ acting on a sequence $x,$ whose $n$th member is called $\Lambda(x)(n),$ as follows:
...
\subsection{The $\phi$-$\rho$ method}

Brute-forcing the No-2 Theorem did leave us with some useful tools to tackle future proofs, but the time taken to brute-force a proof will grow exponentially with the length of the sequence to check for. We must develop a better way. The $\phi$-$\rho$ method will reduce checking a sequence of length $x$ to checking for four sequences of length $y$ such that $3x/4 < y < x.$

We see that the sequence $T_3$ is made up of blocks of 4, for every multiple of 4 $n,$ of $T_3(n), T_3(n+1), T_3(n+2), T_3(n+3),$ which is equal to:

$$T(3n), T(3n+3), T(3n+6), T(3n+9).$$

Recall that the value $T(x)$ is computed using the number of ones in the binary expansion of $x:$ an even number if $T(x) = 0,$ and an odd number is $T(x) = 1.$ Remembering that $n$ is a multiple of 4, we will denote $3n/4$ by $m,$ and denote the binary expansion of any number $x$ by $[x].$ In that case, we have the following binary expansions of the four numbers above:
...
Therefore, we know their values in the Thue-Morse sequence.
...
Because this sequence in $T_3$ repeats every four with a new $m,$ for any sequence $X$ we wish to check, we need only match each element in X to the corresponding entries in the infinite sequence

$$T(m), T(m), \overline{T(m+1)}, \overline{T(m+2)}, T(m+3), T(m+3), \overline{T(m+4)}, \overline{T(m+5)}, \cdots$$

starting at each of the first four entries. Removing the overlines and duplicate entries then gives us four sequences in $T$ to check, each of which are at most as long as $X.$

For example, we can now prove the No-2 Theorem without resorting to brute-forcing. Remembering that the No-2 Theorem implies the existence of $0, 1, 1, 0$ or $1, 0, 0, 1$ in $T_3,$ we can reason about where these sequences must appear. Because $T_3(4n) = T_3(4n + 1)$ for all natural numbers $n,$ we can eliminate any possibility where the first two digits \textbf{01}10 \textbf{10}01 or the last two digits 01\textbf{10} 10\textbf{01} are at positions $4n, 4n+1.$ Therefore, any $0110$ or $1001$ sequence would have to begin at position $4n+1$ or $4n+3.$ We can make a table for each of the two possibilities, where the first two rows are the direct values in $0110$ and $1001$ and the last two rows contain the sequence in $T$ we wish to look for. In fact, due to Lemma \ref{xiffxbar}, we need only look for $0110.$
...
In either case, we have three consecutive entries of the same value in the Thue-Morse sequence, which as per Lemma \ref{triplet} never exists. Therefore, No-2 is proven again. $\Box$

\subsection{No-4, No-5, and No-9+}

\begin{theorem}
Motifs in $T_3$ may only be of lengths 1, 3, 6, 7, or 8.
\end{theorem}

To prove this, we need to prove that no other motifs exist. No-2 has been proven, so we will break this into three parts: proving that no motifs of length 4 exist, proving that none of length 5 exist, and proving that none of length 9 or greater exist.

\subsubsection{No-4}

\begin{theorem}[No-4 Theorem]
...
A sequence of $000$ (which starts at $T(m)$) is disallowed in Thue-Morse, so Case 2 is also ruled out.
...
Such a subsequence is also impossible. Recall that the Thue-Morse sequence is composed of two repeating blocks of length $2^k,$ which are opposites of each other. For $k = 1,$ these two sequences are $01$ and $10.$ So there are two cases.
...
$T(n) = T(2n).$
\end{lemma}
\textbf{Proof} The binary representation of $2n$ is the binary representation of $n$ with a $0$ appended to the end. As per our definition of the values off the Thue-Morse sequence, the values $T(n)$ and $T(2n)$ are therefore equal. $\Box$

In sub-case 1, therefore, we have $T(m) = T(m+2) = T(m+4) = 1,$ and because $m$ is even we may divide by 2 to get $T(m/2) + T(m/2 + 1) = T(m/2 + 2) = 1.$ We know that no sequences of $000$ or $111$ may exist in $T,$ so sub-case 1 is not allowed to exist. Using similar logic for sub-case 2, we see that it is also impossible. Therefore, Case 3 as a whole is impossible.
...
\subsubsection{No-9+ Theorem}
The authors observed that for the first $10^{10}$ terms of $T_3,$ there are no motifs of lengths 9 or greater, which can be proven formally.

\begin{theorem}[No-9+ Theorem]
\label{no9}
There are no motifs of length $9$ or greater in $T_3$.
\end{theorem}

Proving this requires us only to check for a \emph{run} of length 9 or greater. That is, we need to check for $000000000$ or $111111111$ (and by Lemma \ref{xiffxbar}, only $000000000.$) This is because a motif is defined as a run not contained inside any other run. That means:
- Any run of length 9 is either contained inside a motif of length 9 or a motif of length greater than 9. This means that $000000000$ exists only if a counterexample to the No-9 Theorem exists.
- Any motif of length 9 or greater must contain at least one run of length 9. This means that a counterexample to the No-9 Theorem exists only if $000000000$ exists.
- Because we have established an implication in both directions, $000000000$ existing is equivalent to No-9 being false.

We use the $\phi$-$\rho$ method to check all 4 possible cases.
...
This resulting in the same sequence $0110110$ as before, we see that it is also not allowed.
...
The sequence $1101101$ is also disallowed for the same reason -- both possible demarcations into blocks of length 2, which are $1][10][11][01]$ and $[11][01][10][1,$ contain a block of $11.$
...
\begin{conjecture}[Ratio Conjecture]
\label{ratio}
$r(n)$ is never less than $1.$
\end{conjecture}
...
\subsection{Reversing $\phi$-$\rho$}

In Section 2, we developed the notion of the $\phi$-$\rho$ method, which may be thought of more generally as a \emph{transformation} from a sequence in $T_3$ to one or more possible sequences in $T$ which would be its equivalent. However, we can also develop the opposite notion -- a transformation from $T$ to $T_3.$ Of course this is already possible:

$$[0]11[0]10[0]11[0]01[0] \cdots \rightarrow 00000 \cdots .$$

But we can invert $\phi$-$\rho$ to obtain a more useful transformation which uses \emph{every} digit in $T$ to create the corresponding sequence in $T_3.$ We may then apply this transformation to the \emph{whole} of the Thue-Morse sequence to better understand the $T_3$ sequence itself.
...
\begin{lemma}
\label{ratiospec}
$$\hat{\alpha_0}(4n) - \hat{\alpha_1}(4n) = 3 * \hat{\alpha}_0(n) - 3 * \hat{\alpha}_1(n).$$
\end{lemma}
